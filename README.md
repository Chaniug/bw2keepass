...
# KDBX → Bitwarden 密码保护加密 JSON（加密导出，需设导出密码）
python -m bw_to_keepass --reverse input.kdbx output.json --password "your_password" --export-password "export_pwd"

# KDBX → CSV 导出
python -m bw_to_keepass --csv input.kdbx output.csv --password "your_password"
```

> 💡 **加密导出的 salt 模式**：Bitwarden 加密导出对 `salt` 的处理在不同版本/客户端间存在分歧——官方文档为 base64 解码字节（`--salt-mode base64`，可移植性好），个别导出为 salt 字符串的 UTF-8 字节（`--salt-mode utf8`，默认值，对齐常见导出）。若导入 Bitwarden 时提示密码错误或无法识别，换用另一种模式重试即可。导出本身均使用 AES-CBC-256 + HMAC-SHA256（encType 2），与 Bitwarden 规范一致。
...
| `--csv` | CSV 导出（KDBX → CSV） |
| `--export-password` | 反向导出时：将输出的 Bitwarden JSON 加密为该密码保护的加密导出（KDBX → 加密 JSON） |
| `--salt-mode` | 加密导出的 salt 处理方式：`utf8`（默认，对齐常见 Bitwarden 导出）/ `base64`（Bitwarden 文档标准，可移植性更好） |
